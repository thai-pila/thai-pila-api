import pool from "./db";
export interface CreateSituationQuestionInput {
	no: number;
...
export interface CreateSituationAssetInput {
	asset?: string | null;
	position: string;
}

export interface UpdateSituationAssetInput {
	asset?: string | null;
	position?: string;
}
...
type QuestionSituationMediaField = (typeof QUESTION_SITUATION_MEDIA_FIELDS)[number];
type ChoiceSituationMediaField = (typeof CHOICE_SITUATION_MEDIA_FIELDS)[number];

function toQuestionSituationMediaField(field: string): QuestionSituationMediaField {
	if ((QUESTION_SITUATION_MEDIA_FIELDS as readonly string[]).includes(field)) {
		return field as QuestionSituationMediaField;
	}

	throw new Error("invalid question_situation media field");
}

function toChoiceSituationMediaField(field: string): ChoiceSituationMediaField {
	if ((CHOICE_SITUATION_MEDIA_FIELDS as readonly string[]).includes(field)) {
		return field as ChoiceSituationMediaField;
	}

	throw new Error("invalid choice_situation media field");
}
...
export async function createSituationQuestionMultipleWithChoices(idGameInfo: number, questions: CreateSituationQuestionInput[]) {
	...
	try {
		...
		for (const questionItem of questions) {
			...
			const questionRes = await client.query(
                ...
                RETURNING id, id_game_info, no, question, question_position, hint, sound_question, image_question, sound_hint, image_hint, shuffle_answer, skill_listening_speaking, skill_lang_nature, skill_reading, skill_writing, audio_question, audio_choice, audio_assistant_voice, audio_background, expected_time, create_at
                `,
				[
					idGameInfo,
					questionItem.no,
					questionItem.question ?? null,
					questionItem.question_position ?? null,
...
export async function getSituationQuestionsByGameId(idGameInfo: number) {
	...
	try {
		const questionRes = await client.query(
			`
			SELECT id, id_game_info, no, question, question_position, hint, sound_question, image_question, sound_hint, image_hint, shuffle_answer, skill_listening_speaking, skill_lang_nature, skill_reading, skill_writing, audio_question, audio_choice, audio_assistant_voice, audio_background, expected_time, create_at
...
export async function getAllSituationQuestions() {
	const client = await pool.connect();
	try {
		const questionRes = await client.query(
			`
...
export async function updateSituationQuestionMultipleById(questionId: number, payload: UpdateQuestionInput) {
	...
	try {
		...
		if (Object.prototype.hasOwnProperty.call(payload, "no") && payload.no !== currentQuestion.no) {
			...
			if ((conflict.rowCount ?? 0) > 0 && conflict.rows[0].id !== questionId) {
				...
				await client.query(
					`UPDATE situation_question_multiple
		...
		if (Object.prototype.hasOwnProperty.call(payload, "question_position")) {
			updates.push(`question_position = $${idx++}`); values.push(payload.question_position ?? null);
		}
		...
		const updated = await client.query(
			`SELECT id, id_game_info, no, question, question_position, hint, sound_question, image_question, sound_hint, image_hint, shuffle_answer, skill_listening_speaking, skill_lang_nature, skill_reading, skill_writing, audio_question, audio_choice, audio_assistant_voice, audio_background, expected_time, create_at, update_at
			FROM situation_question_multiple
			WHERE id = $1`,
			[questionId]
		);
...
export interface BulkUpdateSituationChoiceInput {
	id: number;
	choice?: string | null;
	is_correct?: boolean;
	sound_choice?: string | null;
...
export async function updateSituationChoicesByQuestionId(idQuestionMultiple: number, choices: BulkUpdateSituationChoiceInput[]) {
	...
	try {
		await client.query("BEGIN");
		const questionRes = await client.query(`SELECT id FROM situation_question_multiple WHERE id = $1 AND delete_at IS NULL`, [idQuestionMultiple]);
		if (questionRes.rowCount === 0) throw new Error("question not found");

		const existingChoicesRes = await client.query(`SELECT id FROM situation_choice_multiple WHERE id_question_multiple = $1 AND delete_at IS NULL`, [idQuestionMultiple]);
		...
		for (const item of choices) {
			...
			const sql = `
                UPDATE situation_choice_multiple
                SET ${updates.join(", ")}, update_at = CURRENT_TIMESTAMP
			...
			if (updated.rowCount === 0) throw new Error(`failed to update choice id ${item.id}`);
...
export async function createSituationChoicesByQuestionId(
	...
	try {
		...
		const questionRes = await client.query(
			`SELECT id FROM situation_question_multiple WHERE id = $1 AND delete_at IS NULL`,
			[idQuestionMultiple]
		);
		if (questionRes.rowCount === 0) throw new Error("question not found");
...
export async function softDeleteSituationQuestionById(questionId: number) {
	if (!Number.isInteger(questionId) || questionId <= 0) {
		throw new Error("invalid question id");
	}
	const client = await pool.connect();
	try {
		...
		const updatedChoices = await client.query(
			`UPDATE situation_choice_multiple SET delete_at = CURRENT_TIMESTAMP WHERE id_question_multiple = $1 AND delete_at IS NULL RETURNING id`,
			[questionId]
		);

		const updatedQuestion = await client.query(
			`UPDATE situation_question_multiple SET delete_at = CURRENT_TIMESTAMP WHERE id = $1 AND delete_at IS NULL RETURNING id`,
			[questionId]
		);
...
export async function hardDeleteSituationQuestionById(questionId: number) {
	if (!Number.isInteger(questionId) || questionId <= 0) {
		throw new Error("invalid question id");
	}
	const client = await pool.connect();
	try {
		...
		const questionRes = await client.query(
			`SELECT id FROM situation_question_multiple WHERE id = $1`,
			[questionId]
		);
		if (questionRes.rowCount === 0) {
			throw new Error("question not found");
		...
		const deletedChoices = await client.query(
			`DELETE FROM situation_choice_multiple WHERE id_question_multiple = $1 RETURNING id`,
			[questionId]
		);

		const deletedQuestion = await client.query(
			`DELETE FROM situation_question_multiple WHERE id = $1 RETURNING id`,
			[questionId]
		);
...
export async function hardDeleteSituationChoicesByIds(questionId: number, choiceIds: number[]) {
	if (!Number.isInteger(questionId) || questionId <= 0) {
		throw new Error("invalid question id");
	}
	...
	try {
		...
		const deletedRes = await client.query(
			`DELETE FROM situation_choice_multiple
			 WHERE id_question_multiple = $1 AND id = ANY($2::int[])
			 RETURNING id`,
			[questionId, normalizedChoiceIds]
		);
...
export async function createSituationAssetsByGameId(idGameInfo: number, assets: CreateSituationAssetInput[]) {
	if (!Number.isInteger(idGameInfo) || idGameInfo <= 0) {
		throw new Error("id_game_info must be a positive integer");
	}
	...
	for (const assetItem of assets) {
		if (!assetItem || typeof assetItem !== "object") {
			throw new Error("each asset item must be object");
		}

		if (!assetItem.position || typeof assetItem.position !== "string" || assetItem.position.trim() === "") {
			throw new Error("each asset.position is required");
		}
	}
	...
	try {
		...
		for (const assetItem of assets) {
			const insertRes = await client.query(
				...
				VALUES ($1, $2, $3)
				RETURNING id, id_game_info, asset, position, create_at, update_at, delete_at
				`,
				[idGameInfo, withUploadPrefix(assetItem.asset ?? null) ?? null, assetItem.position.trim()]
			);

			createdAssets.push(insertRes.rows[0]);
		}
...
export async function updateSituationAssetById(assetId: number, payload: UpdateSituationAssetInput) {
	...
	if (Object.prototype.hasOwnProperty.call(payload, "asset")) {
		updates.push(`asset = $${idx++}`);
		values.push(withUploadPrefix(payload.asset ?? null) ?? null);
	}

	if (Object.prototype.hasOwnProperty.call(payload, "position")) {
		if (!payload.position || typeof payload.position !== "string" || payload.position.trim() === "") {
			throw new Error("position must be a non-empty string");
		}

		updates.push(`position = $${idx++}`);
		values.push(payload.position.trim());
	}
...
export async function hardDeleteSituationAssetById(assetId: number) {
	if (!Number.isInteger(assetId) || assetId <= 0) {
		throw new Error("invalid asset id");
	}

	const res = await pool.query(
		`DELETE FROM situation_assets WHERE id = $1 RETURNING id, id_game_info, asset, position`,
		[assetId]
	);
...
export async function hardDeleteQuestionSituationMediaFieldByQuestionId(questionId: number, field: string) {
	if (!Number.isInteger(questionId) || questionId <= 0) {
		throw new Error("invalid question id");
	}

	const targetField = toQuestionSituationMediaField(field);
	...
	const result = await pool.query(
		...
		WHERE id = $1 AND delete_at IS NULL
		RETURNING id, id_game_info, no, question, question_position, hint, sound_question, image_question, sound_hint, image_hint, shuffle_answer, skill_listening_speaking, skill_lang_nature, skill_reading, skill_writing, audio_question, audio_choice, audio_assistant_voice, audio_background, expected_time, create_at, update_at
		`,
		[questionId]
	);
	...
	return {
		id_situation_question_multiple: questionId,
		deleted_field: targetField,
		question: result.rows[0],
	};
}

export async function hardDeleteChoiceSituationMediaFieldByChoiceId(choiceId: number, field: string) {
	if (!Number.isInteger(choiceId) || choiceId <= 0) {
		throw new Error("invalid choice id");
	}

	const targetField = toChoiceSituationMediaField(field);
	...
	return {
		id_situation_choice_multiple: choiceId,
		deleted_field: targetField,
